...
io.on("connection", (socket) => {
	...
	socket.on("register", function (data) {
		switch (data.role) {
			case "user":
				userid[data.address] = socket.id;
				break;
			case "newsfeed":
				newsfeedid[data.address] = socket.id;
				break;
			case "comment":
				commentid[data.objecthash] = socket.id;
				break;
			default: break;
		}
	...
	fetchData.fetch()
		.then(() => {
			...
			ws.onopen = function (e) {
				...
				ws.send(JSON.stringify({ "method": "subscribe", "params": { "query": "tm.event='NewBlock'" }, "id": 0 }));
			};
			ws.onmessage = function (e) {
				console.log("NewBlock received");
